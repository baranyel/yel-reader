...
function TextBlock({ block, onChange, onDelete, onAddTextAfter, onAddImageAfter, t, fileInputRef, setPendingInsert, blockIndex }) {
  ...
  return (
    <div style={{ position: 'relative', marginBottom: 4 }}>
      <textarea
        ...
        placeholder={t('noteEditor.text_ph')}
        rows={3}
        style={{
          width: '100%', resize: 'none', overflow: 'hidden',
          border: '1.5px solid var(--border)', borderRadius: 'var(--radius-sm)',
          background: 'var(--bg-sunk)', padding: '10px 40px 10px 12px',
          fontSize: 14.5, lineHeight: 1.65, color: 'var(--text)',
          fontFamily: 'var(--reading-font)', outline: 'none',
...
function ImageBlock({ block, imageUrl, onCaptionChange, onDelete, onAddTextAfter, onAddImageAfter, t, fileInputRef, setPendingInsert, blockIndex }) {
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{ position: 'relative', borderRadius: 'var(--radius-sm)', overflow: 'hidden', border: '1.5px solid var(--border)', background: 'var(--bg-sunk)' }}>
        {imageUrl ? (
          ...
          <div style={{ height: 80, display: 'grid', placeItems: 'center', color: 'var(--text-faint)', fontSize: 13 }}>
            <Icon name="loader" size={20} />
          </div>
        )}
        <button onClick={() => onDelete(block.id, block.imageId)} style={{
      ...
      <input
        ...
        style={{
          width: '100%', border: '1.5px solid var(--border)', borderTop: 'none',
          borderRadius: '0 0 var(--radius-sm) var(--radius-sm)',
          background: 'var(--bg-sunk)', padding: '7px 12px',
          fontSize: 13, color: 'var(--text-faint)', outline: 'none',
          fontStyle: 'italic', transition: 'border-color .15s',
        }}
        onFocus={(e) => e.target.style.borderColor = 'var(--accent)'}
        onBlur={(e) => e.target.style.borderColor = 'var(--border)'}
      />
...
export default function NoteEditor({ note, paragraphPreview, textId, textTitle, paragraphIndex, onSave, onClose, onDelete, t }) {
  ...
  const [pendingInsert, setPendingInsert] = useState(null); // block index after which to insert
  const fileInputRef = useRef(null);

  // Load image data URLs for existing image blocks
  useEffect(() => {
    const load = async () => {
      const urls = {};
      for (const b of blocks) {
        if (b.type === 'image' && b.imageId) {
          urls[b.imageId] = await getImage(b.imageId);
        }
      }
      setImageUrls(urls);
    };
  ...
  const handleDelete = () => {
    if (window.confirm(t('notes.confirm_delete'))) onDelete();
  };
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          ...
          <div style={{
            fontSize: 13, fontFamily: 'var(--reading-font)', fontStyle: 'italic',
